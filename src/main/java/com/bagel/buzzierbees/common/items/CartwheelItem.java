...
public class CartwheelItem extends BlockItem {
	...
	public ActionResultType onItemUse(ItemUseContext context) {
		...
		if (blockstate.getBlock() == Blocks.FLOWER_POT) {
			if (!world.isRemote) {
				world.setBlockState(context.getPos(), ModBlocks.POTTED_CARTWHEEL.getDefaultState().with(PottedCartwheelBlock.FACING, context.getPlacementHorizontalFacing().getOpposite()), 11);
