...
public class NewPistonBlock extends DirectionalBlock  {
    ...
    public NewPistonBlock(boolean sticky, Block.Properties properties) {
        super(properties);
        ...
        if (flag && !state.get(EXTENDED)) {
            ...
            if (blockstate.getBlock() == Blocks.MOVING_PISTON && blockstate.get(FACING) == direction) {
                ...
                if (tileentity instanceof NewPistonTileEntity) {
                    NewPistonTileEntity pistontileentity = (NewPistonTileEntity)tileentity;
    ...
    public boolean eventReceived(BlockState state, World worldIn, BlockPos pos, int id, int param) {
        ...
        if (id == 0) {
            ...
            if (tileentity1 instanceof NewPistonTileEntity) {
                ((NewPistonTileEntity)tileentity1).clearPistonTileEntity();
            }

            worldIn.setBlockState(pos, ModBlocks.MOVING_PISTON.getDefaultState().with(NewMovingPistonBlock.FACING, direction).with(NewMovingPistonBlock.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT), 3);
            worldIn.setTileEntity(pos, NewMovingPistonBlock.createTilePiston(this.getDefaultState().with(FACING, Direction.byIndex(param & 7)), direction, false, true));
            if (this.isSticky) {
    ...
    private boolean doMove(World worldIn, BlockPos pos, Direction directionIn, boolean extending) {
        ...
        if (!pistonblockstructurehelper.canMove()) {
            ...
            for(int l = list.size() - 1; l >= 0; --l) {
                ...
                worldIn.setTileEntity(blockpos3, NewMovingPistonBlock.createTilePiston(list1.get(l), directionIn, extending, false));
                --k;
            ...
            if (extending) {
                ...
                BlockState blockstate4 = ModBlocks.PISTON_HEAD.getDefaultState().with(PistonHeadBlock.FACING, directionIn).with(PistonHeadBlock.TYPE, pistontype);
                BlockState blockstate6 = ModBlocks.MOVING_PISTON.getDefaultState().with(NewMovingPistonBlock.FACING, directionIn).with(NewMovingPistonBlock.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT);
                map.remove(blockpos);
                worldIn.setBlockState(blockpos, blockstate6, 68);
                worldIn.setTileEntity(blockpos, NewMovingPistonBlock.createTilePiston(blockstate4, directionIn, true, true));
            }
