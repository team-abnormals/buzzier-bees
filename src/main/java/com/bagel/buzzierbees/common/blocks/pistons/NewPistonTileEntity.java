...
public class NewPistonTileEntity extends TileEntity implements ITickableTileEntity {
    ...
    public NewPistonTileEntity() {
        super(ModTileEntities.PISTON.get());
    }

    public NewPistonTileEntity(BlockState pistonStateIn, Direction pistonFacingIn, boolean extendingIn, boolean shouldHeadBeRenderedIn) {
        this();
    ...
    private BlockState getCollisionRelatedBlockState() {
        return !this.isExtending() && this.shouldPistonHeadBeRendered() && this.pistonState.getBlock() instanceof NewPistonBlock ? ModBlocks.PISTON_HEAD.getDefaultState().with(PistonHeadBlock.TYPE, this.pistonState.getBlock() == Blocks.STICKY_PISTON ? PistonType.STICKY : PistonType.DEFAULT).with(PistonHeadBlock.FACING, this.pistonState.get(NewPistonBlock.FACING)) : this.pistonState;
    }
    ...
    private boolean func_227025_y_() {
        return this.pistonState.getBlock() == ModBlocks.HONEY_BLOCK || this.pistonState.getBlock() == ModBlocks.CLOVER_HONEY_BLOCK;
    }
    ...
    public VoxelShape getCollisionShape(IBlockReader p_195508_1_, BlockPos p_195508_2_) {
        ...
        if (!this.extending && this.shouldHeadBeRendered) {
            voxelshape = this.pistonState.with(NewPistonBlock.EXTENDED, Boolean.valueOf(true)).getCollisionShape(p_195508_1_, p_195508_2_);
        } else {
