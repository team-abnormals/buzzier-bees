...
@OnlyIn(Dist.CLIENT)
public class BearEyesRenderLayer<E extends AbstractBearEntity, M extends EntityModel<E>> extends LayerRenderer<E, M> {
	private static final Map<List<String>, String> NAMES = Util.make(Maps.newHashMap(), (skins) -> {
		skins.put(Arrays.asList("derp", "byper"), "derp");
	});
	
	public BearEyesRenderLayer(IEntityRenderer<E, M> entityRenderer) {
	...
	public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, E bear, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {		
		...
		if (bear instanceof BlackBearEntity && bear.isSleeping()) overlay = "black_bear_eyes_sleeping"; 
		if (bear instanceof GrizzlyBearEntity && bear.isSleeping()) overlay = "grizzly_bear_eyes_sleeping"; 
		if (overlay != "") {
